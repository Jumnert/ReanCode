...
const DEFAULT_CODE = `// សួស្តី TypeScript! 👋
let name: string = "ពិភពលោក";
...
console.log("ផលបូក:", add(10, 5));`

type LogLine = { type: "log" | "info" | "warn" | "error"; text: string }

interface TsCompilerProps {
  defaultCode?: string
  className?: string
...
export function TsCompiler({ defaultCode = DEFAULT_CODE, className, compact = false, fullHeight = false, onChange }: TsCompilerProps) {
...
<script type="text/babel" data-presets="typescript">
  try {
    ...
    console.error(e && e.message ? (e.name + ": " + e.message) : String(e));
  }
</script></body></html>`
    ...
    >
      ...
      <div className={cn("flex items-center justify-between px-3 py-2 border-b transition-colors", toolbar)}>
        <div className="flex items-center gap-2">
          ...
          <div className={cn("ml-3 text-xs font-medium tracking-wide", toolbarText)}>script.ts</div>
        </div>
