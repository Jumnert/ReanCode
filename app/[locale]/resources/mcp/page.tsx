...
export default function MCPResourcesPage() {
  const mcpResources = resources.filter((r) => r.category === "mcp" || r.category === "python");
      ...
      <div className="max-w-[1440px] mx-auto border-x-2 border-[#cc785c]/20 relative">
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center px-8 pt-12 pb-16 relative z-10">
          ...
          <div className="flex flex-col items-start space-y-6">
            ...
            >
              Model Context <br/><span className="text-[#cc785c]">Protocols</span>
            </motion.h1>
            <motion.p 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              Empower AI models with external capabilities. Connect agents securely to databases, filesystems, and APIs via standardized server architectures.
            </motion.p>
          </div>
                ...
                <AccordionContent className="w-full bg-[#efe9de]/30 dark:bg-[#181715]/30 m-0 border-t-0 data-[state=closed]:border-t-0">
                  <div className="w-full max-w-[1440px] mx-auto px-12 py-6 text-left">
                    <p className="text-[17px] text-[#3d3d3a] dark:text-zinc-400 max-w-4xl italic leading-relaxed border-l-4 border-[#cc785c] pl-6">
                      "Model Context Protocol (MCP) servers allow AI agents to safely interface with external data sources like databases, file systems, and APIs. By using an MCP, you abstract away complex integration and authentication logic, giving your agent direct, secure access to the tools it needs to accomplish complex real-world tasks."
                    </p>
                  </div>
                </AccordionContent>
