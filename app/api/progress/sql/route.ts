...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Ensure the SQL course exists
    let course = await prisma.course.findUnique({ where: { slug: "sql" } });
    if (!course) {
      course = await prisma.course.create({
        data: {
          slug: "sql",
          title: "SQL Course",
          description: "Basic SQL Course",
          category: "sql",
          published: true,
        }
      });
    ...
    console.error("Error in POST /api/progress/sql:", error);
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get all completed SQL lessons for this user
    ...
    console.error("Error in GET /api/progress/sql:", error);
    return ApiResponse.serverError();
  }
}
