...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Ensure the JavaScript course exists
    let course = await prisma.course.findUnique({ where: { slug: "javascript" } });
    if (!course) {
    ...
    console.error("Error in POST /api/progress/javascript:", error);
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get all completed JavaScript lessons for this user
    ...
    console.error("Error in GET /api/progress/javascript:", error);
    return ApiResponse.serverError();
  }
}
